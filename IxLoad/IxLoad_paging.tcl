...
# IxLoad_paging.tcl made on Jan 16 2018 17:29
#################################################
...
$ixAppPluginManager load "stateless"
...
#################################################
set Subscriber1_Network1 [$scenarioElementFactory create $::ixScenarioElementType(kSubscriberNetTraffic)]



#################################################
# Network Network1 of NetTraffic Subscriber1@Network1
#################################################
set Network1 [$Subscriber1_Network1 cget -network]
...
$Network1 portList(0).setPortCaptureFileName "RESULTS\\IxLoad_paging\\Port_1_1_2_capture.cap"
...
set HTTPClient1_Traffic1_HTTPServer1 [::IxLoad new ixNetMeshingTrafficMap]

# ixNet objects need to be added in the list before they are configured!
$Meshing_1 trafficMaps.appendItem -object $HTTPClient1_Traffic1_HTTPServer1



$HTTPClient1_Traffic1_HTTPServer1 config \
	-ipPreference                            2 \
	-name                                    "HTTPClient1!Traffic1_HTTPServer1" \
	-portRangesString                        "" \
...
$Network1 activePortList.appendItem \
	-chassisId                               1 \
	-cardId                                  1 \
	-portId                                  2 
...
set VLAN_R6 [$IP_R5 getLowerRelatedRange "VlanIdRange"]
...
set IP_R4 [$eNB_R2 getLowerRelatedRange "IpV4V6Range"]

$IP_R4 config \
	-count                                   1 \
	-enableGatewayArp                        false \
	-randomizeSeed                           2287221227 \
	-generateStatistics                      false \
...
set VLAN_R5 [$IP_R4 getLowerRelatedRange "VlanIdRange"]
...
#################################################
$Subscriber1_Network1 config \
	-enable                                  1 \
	-tcpAccelerationAllowedFlag              true \
	-network                                 $Network1 

#################################################
# Network Activities for Subscriber1@Network1
#################################################
set Activity_eGTP_Control_Plane [$Subscriber1_Network1 networkActivityList.getNetworkActivityByPluginType $ENB "Control Plane"]

#################################################
# Timeline3 for activities HTTPClient1, eGTP Control Plane
#################################################
set Timeline3 [::IxLoad new ixTimeline]
...
$Activity_eGTP_Control_Plane agent.pm.commands.appendItem \
	-commandType                             "THINK" \
...
$Activity_eGTP_Control_Plane agent.pm.commands.appendItem \
	...
	-subType                                 "egtpEnterIdleState" 

$Activity_eGTP_Control_Plane agent.pm.commands(1).config \
	-displayName                             "Enter Idle State" \
	-_Stale                                  false 
...
#################################################
# Subscriber Activity HTTPClient1 of NetTraffic Subscriber1@Network1
#################################################
set Subscriber_Activity_HTTPClient1 [$Subscriber1_Network1 activityList.appendItem -protocolAndType "HTTP Client"]
...
set DedicatedBearer_2 [::IxLoad new ixNetEGTPCommandAPNBearer]
...
$my_ixThinkCommand config \
	-commandType                             "THINK" \
	-minimumInterval                         12500 \
	-maximumInterval                         12500 \
	-cmdName                                 "Think 3" 
...
$my_ixHttpCommand config \
	...
	-pingFreq                                10 \
	-streamIden                              3 \
	-destination                             "Traffic1_HTTPServer1:80" \
...
$Subscriber1_Network1 traffic.config -name "Subscriber1"

$Subscriber1_Network1 setPortOperationModeAllowed $::ixPort(kOperationModeThroughputAcceleration) false

$Subscriber1_Network1 setPortOperationModeAllowed $::ixPort(kOperationModeFCoEOffload) true

$Subscriber1_Network1 setPortOperationModeAllowed $::ixPort(kOperationModeL23) true

$Subscriber1_Network1 setTcpAccelerationAllowed $::ixAgent(kTcpAcceleration) true

$Originate elementList.appendItem -object $Subscriber1_Network1
...
#################################################
set Traffic1_Network2 [$scenarioElementFactory create $::ixScenarioElementType(kNetTraffic)]



#################################################
# Network Network2 of NetTraffic Traffic1@Network2
#################################################
set Network2 [$Traffic1_Network2 cget -network]
...
$Network2 portList(0).setPortCaptureEnable true

$Network2 portList(0).setPortCaptureFileName "RESULTS\\IxLoad_paging\\Port_1_1_3_capture.cap"
...
$Network2 activePortList.appendItem \
	-chassisId                               1 \
	-cardId                                  1 \
	-portId                                  3 
...
# ixNet objects need to be added in the list before they are configured!
$Ethernet_2 childrenList.appendItem -object $MAC_VLAN_8
...
set IP_6 [::IxLoad new ixNetIpV4V6Plugin]

# ixNet objects need to be added in the list before they are configured!
$MAC_VLAN_8 childrenList.appendItem -object $IP_6



$IP_6 childrenList.clear



$IP_6 extensionList.clear



$IP_6 config -_Stale false

$MAC_VLAN_8 extensionList.clear



$MAC_VLAN_8 config -_Stale false
...
# Setting the ranges starting with the plugins that need to be script gen first
#################################################
$IP_6 rangeList.clear

set IP_R6 [::IxLoad new ixNetIpV4V6Range]

# ixNet objects need to be added in the list before they are configured.
$IP_6 rangeList.appendItem -object $IP_R6
...
$MAC_R6 config \
	...
	-enabled                                 true \
	-mtu                                     1500 \
	-mac                                     "00:AC:20:02:0F:00" \
	-_Stale                                  false \
	-incrementBy                             "00:00:00:00:00:01" 

set VLAN_R7 [$IP_R6 getLowerRelatedRange "VlanIdRange"]
...
# ixNet objects need to be added in the list before they are configured!
$IP_6 rangeGroups.appendItem -object $DistGroup1



# ixNet objects need to be added in the list before they are configured.
$DistGroup1 rangeList.appendItem -object $IP_R6
...
$Traffic1_Network2 config \
	-enable                                  true \
	-tcpAccelerationAllowedFlag              true \
	-network                                 $Network2 

#################################################
# Activity HTTPServer1 of NetTraffic Traffic1@Network2
#################################################
set Activity_HTTPServer1 [$Traffic1_Network2 activityList.appendItem -protocolAndType "HTTP Server"]
...
$_200_OK config \
	...
	-lastModifiedDateTimeValue               "2017/11/08 10:50:32" \
	-lastModifiedIncrementFor                1 \
	-expirationAfterLastModifiedValue        3600 \
	-dateTimeValue                           "2017/11/08 10:50:32" \
	-dateZone                                "GMT" \
	-dateMode                                2 \
	-expirationAfterRequestValue             3600 \
	-dateIncrementBy                         5 \
	-expirationDateTimeValue                 "2017/12/08 10:50:32" \
...
$Traffic1_Network2 traffic.config -name "Traffic1"

$Traffic1_Network2 setPortOperationModeAllowed $::ixPort(kOperationModeThroughputAcceleration) false

$Traffic1_Network2 setPortOperationModeAllowed $::ixPort(kOperationModeFCoEOffload) true

$Traffic1_Network2 setPortOperationModeAllowed $::ixPort(kOperationModeL23) true

$Traffic1_Network2 setTcpAccelerationAllowed $::ixAgent(kTcpAcceleration) true

$Terminate elementList.appendItem -object $Traffic1_Network2
...
#################################################
set destination [$Subscriber1_Network1 getDestinationForActivity "HTTPClient1" "Traffic1_HTTPServer1"]
...
$testController setResultDir "RESULTS\\IxLoad_paging"

$Subscriber1_Network1 setCommunityCaptureParams $Test1 true

$Network1 portList(0).setAnalyzerPartialCapture "False;8192"

$Traffic1_Network2 setCommunityCaptureParams $Test1 true

$Network2 portList(0).setAnalyzerPartialCapture "False;8192"
...
$Subscriber1_Network1 removeAllPortsFromAnalyzer

$Traffic1_Network2 removeAllPortsFromAnalyzer
...
::IxLoad delete $Subscriber1_Network1
...
::IxLoad delete $HTTPClient1_Traffic1_HTTPServer1
...
::IxLoad delete $Traffic1_Network2
